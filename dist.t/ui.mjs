...
class CapoDAppProvider extends Component {
  ...
  async connectWallet(autoNext = true, retries = 5) {
    ...
    if (selectedWallet === "zwallet") {
      let numberArrayToUint8Array2 = function(arr) {
        return new Uint8Array(arr);
      }, uint8ArrayToNumberArray2 = function(arr) {
        return Array.from(arr);
      };
      let placeholderPassword = "qwerty";
      ...
      const encryptedKeyData = window.localStorage.getItem("zwk");
      let privKeyHex;
      if (!encryptedKeyData) {
        const entropy = makeRandomRootPrivateKey().entropy;
        const entropyUint8 = numberArrayToUint8Array2(entropy);
        const salt = crypto.getRandomValues(new Uint8Array(16));
        const key = await deriveKey(placeholderPassword, salt);
        const { encrypted, iv } = await encryptData(entropyUint8, key);
        const storageData = {
          salt: bytesToHex(uint8ArrayToNumberArray2(salt)),
          iv: bytesToHex(uint8ArrayToNumberArray2(iv)),
          encrypted: bytesToHex(uint8ArrayToNumberArray2(encrypted))
        };
        window.localStorage.setItem("zwk", JSON.stringify(storageData));
        privKeyHex = bytesToHex(entropy);
      } else {
        let storageData;
        try {
          storageData = JSON.parse(encryptedKeyData);
          if (storageData.encrypted && storageData.salt && storageData.iv) {
            const saltArray = hexToBytes(storageData.salt);
            const ivArray = hexToBytes(storageData.iv);
            const encryptedArray = hexToBytes(storageData.encrypted);
            const salt = numberArrayToUint8Array2(saltArray);
            const iv = numberArrayToUint8Array2(ivArray);
            const encrypted = numberArrayToUint8Array2(encryptedArray);
            const key = await deriveKey(placeholderPassword, salt);
            const decryptedEntropy = await decryptData(encrypted, iv, key);
            const decryptedEntropyArray = uint8ArrayToNumberArray2(decryptedEntropy);
            privKeyHex = bytesToHex(decryptedEntropyArray);
          } else {
            privKeyHex = encryptedKeyData;
          }
        } catch {
          privKeyHex = encryptedKeyData;
        }
