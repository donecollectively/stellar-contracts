...
- **Testing time-sensitive transactions**: When a policy uses `getTimeRange(granularity)` or `now()`, tests must set the validity window correctly. `tcx.txnTime` is the `validity.start` seen on-chain as `now()`. For how `validFor()`, `txnTime`, and `txnEndTime` synchronize with on-chain time checks, see `essential-stellar-offchain.md` § "Validity windows".
  ...
  - **`futureDate` — specialized use only**: `tcx.futureDate(date)` pins `txnTime` to a specific future time and the emulator fast-forwards to meet it. This is **not** a general-purpose time tool — reserve it for cases where the transaction itself must not be valid until a future moment (e.g., testing that a transaction built now can only execute after a delay, or verifying time-locked release logic). Most time-sensitive tests should use `txnTime` + datum manipulation instead.
