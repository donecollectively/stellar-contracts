...
- Reference Documentation and Agent Guidance
  - **Extensive Reference Library Additions**: New and updated guides cover on-chain Helios patterns, off-chain transaction building, validity windows, byte comparison safety, full-stack architecture, and testing patterns, organised for both human readers and AI-assisted development workflows.
...
- Transaction-Building and On-Chain Data Lifecycle
  - **Activity Introspection**: The `isActivity` type now carries the module, activity name, and underlying data object, useful for `beforeCreate`/`beforeUpdate` hooks that need to vary behaviour by activity type.
...
- Testing Environment
  - **On-Disk State Snapshots**: Test suites no longer re-run expensive blockchain setup on every run. The emulator persists smart contract state, test-scenario actor wallets, and transaction history to disk, restoring in seconds. Only snapshots that have changed rebuild; the rest are reused as-is.
