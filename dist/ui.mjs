...
import { makeBlockfrostV0Client, makeCip30Wallet, makeWalletHelper } from '@helios-lang/tx-utils';
...
class CapoDAppProvider extends Component {
  ...
  render() {
    ...
    const addrInfo = "development" == process.env.NODE_ENV && capo?._compiledScript ? capo.address.toString() : "";
  ...
  async connectWallet(autoNext = true, retries = 5) {
    ...
    const handle = await connecting.catch((e) => {
      this.walletConnectPromise = void 0;
  ...
  async setWallet(details) {
    ...
    if (!collateralUtxos?.length) {
      this.updateStatus(
        ...
        {
          isError: true,
          moreInstructions: "Ensure at least one small-ish pure-ADA utxo (<20 ADA) exists in your wallet.  You may also try marking one or more small-ish (~5 ada) utxos in your wallet as being reserved for collateral",
          developerGuidance: "let the user fix the problem.  Or, you may check for 'no collateral' in the error message and make a txn for creating some collateral < 20 ADA"
        },
        "//no collateral seen"
      );
      return;
  ...
  async checkWalletTokens() {
    ...
    const message = roles.includes("member") ? "" : this.getStartedMessage();
    this.updateStatus(
      message,
      {
        progressPercent: 100,
...
function useCapoDappProvider() {
  const context = React__default.useContext(CapoDappProviderContext);
  if (!context) {
    throw new Error(
...
function CharterHighlights({
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, coreDelegates, manifestNamedEntries, ...Object.entries(capo.delegateRoles).flatMap(
    ([roleName, roleInfo]) => {
      ...
      if (!foundRole)
        return /* @__PURE__ */ React.createElement(DashHighlightItem, { title: roleName }, /* @__PURE__ */ React.createElement(Softlight, null, "Delegated data policy"), /* @__PURE__ */ React.createElement(Highlight, null, "needs deployment"));
      return /* @__PURE__ */ React.createElement(
