...
export type mkDataBridgeType<
    T extends StellarContract<any>,
    bridgeClassMaybe = T["mkDataClass"],
    hasBridge = bridgeClassMaybe extends typeof DataMaker ? InstanceType<bridgeClassMaybe> : never
    // HasDatum extends (ReturnType<T["mkDataClass]"]> extends typeof DataMaker
> = hasBridge;
...
> {
    ...
    /**
     * the mkDataClass attribute MUST be defined for any bundle having a datum type
     ...
     */
    mkDataClass: Option<typeof DataMaker> = null;
     ...
     *
     */
    get activity(): mkDataBridgeType<this>["activity"] {
        return this.mkData.activity;
    }
    /**
     ...
     *
     */
    mkDataBridge?: Option<someDataMaker>; // Option<BundleType<this>["mkDatum"]>
    get onchain() {
        return this.mkData;
    }

    get mkDatum() : mkDataBridgeType<this>["datum"]{
        return this.mkData.datum;
    }

    get mkData(): mkDataBridgeType<this> {
        if ("undefined" == typeof this.mkDataBridge) {
            const datumType = this.getBundle().locateDatumType();
            if (datumType) {
                const { mkDataClass } = this;
                if (!mkDataClass) {
                    throw new Error(
                        `${this.constructor.name} MUST define mkDataClass = mkDataBridge‹YourScriptName›\n` +
                            `  ... this mkDataBridge class should be generated by heliosRollupTypeGen \n` +
                            `  ... and imported (\`import mkDataBridge‹something› from "./‹yourScriptName›.mkData.js"\`)\n` +
                            `      This critical class converts off-chain, typed data for use in the on-chain environment.`
                    );
                }
                console.log(
                    `${this.constructor.name} mkDataClass = `,
                    mkDataClass.name
                );
                try {
                    debugger;
                    this.mkDataBridge = new mkDataClass(
                        this.getBundle()
                    ) as any;
                } catch (e) {
                    console.error(e);
                ...
                console.log(`${this.constructor.name} mkDataClass = NONE`);
                this.mkDataBridge = null;
            }
        }

        debugger;
        if (!this.mkDataBridge) {
            throw new Error(
                `${this.constructor.name}: this contract script doesn't use datum`
            );
        }

        return this.mkDataBridge;
    }

    get newReadDatum(): mkDataBridgeType<this>["readDatum"] {
        const readDatum = this;
        if (!readDatum) {
            throw new Error(
                `${
     ...
     */
    get redeemer(): mkDataBridgeType<this>["activity"] {
