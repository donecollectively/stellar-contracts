...
> {
     ...
     * ... and provides type-safe interfaces for doing that.
     * 
     * @remarks
     ...
     * 
     */
    mkDataBridge?: Option<typeof someDataMaker> // Option<BundleType<this>["mkDatum"]>
    get onchain() {
    ...
    get mkData() { // BundleType<this>["mkDatum"] {
        if ("undefined" == typeof this.mkDataBridge) {
            ...
            if (datumType) {
                ...
                if (!mkDataClass) {
                    throw new Error(`${this.constructor.name} MUST define mkDataClass\n`+
                        `  ... this mkData bridge should be generated by heliosRollupTypeGen \n`+
                        `  ... and imported (\`import mkDataBridge from "...your-bundle-name.mkData.js"\`)\n`+
                        `  ...  This critical class converts off-chain, typed data for use in the on-chain environment.`
                    );
                }
                console.log(`${this.constructor.name} mkDataClass = `, mkDataClass.name)
                this.mkDataBridge = new mkDataClass(this.getBundle()) as any
            } else {
