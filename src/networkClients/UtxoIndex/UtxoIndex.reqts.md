...
Defines how the indexer performs initial synchronization and ongoing transaction monitoring of the Capo address. Applied when implementing sync logic, periodic monitoring, or transaction processing.
 ...
 - **REQT-1.3.2**/fh56sce22g: IMPLEMENTED/NEEDS VERIFICATION: **Transaction Monitoring** — `runSyncCycle()` (was `checkForNewTxns()`) orchestrates chain state synchronization as a sequence of named steps: (1) discover chain tip, (2) detect and handle rollbacks, (3) sync chain state via incremental or catchup mode, (4) verify pending tx confirmations via REQT/ba6eq9x10s (On-Chain Pending Verification), (5) resubmit stale pending txs, (6) update confirmation depths, (7) execute settled rollbacks. MUST reliably discover all transactions touching `capoAddress`, with no gaps from timing, indexing lag, or cursor advancement. MUST process each discovered transaction via `processTransactionForNewUtxos()`. A transaction that lands on-chain MUST eventually be discovered regardless of when the chain tip advances. MUST select sync mode based on how far behind the processing cursor is (configurable threshold, default 20 blocks).
    ...
    - **REQT-1.3.2.2**/2he55bafxd: IMPLEMENTED/NEEDS VERIFICATION: **Catchup Mode** — When the gap exceeds the threshold, MUST use address-level transaction queries (`from=lastProcessedBlock`) to sparsely discover relevant transactions and process each via `processTransactionForNewUtxos()`. MUST NOT enumerate or store intermediate blocks — catchup does not use per-block walks. MUST reconcile cached UTxOs against current on-chain state via address UTxO query to ensure cache correctness. Reconciliation MUST fetch current on-chain UTxOs for the Capo address, then mark any cached unspent UTxO absent from the on-chain set as spent via `spentInTx: "reconciled"`. MUST NOT mark UTxOs as spent if their txHash (extracted from `utxoId`) is present in `pendingTxHashes` — these are speculative outputs from pending transactions not yet confirmed on-chain. DEPENDS on REQT/q2aqr5rh7g (Pending State Before Sync) for `pendingTxHashes` availability. MUST advance the processing cursor to the chain tip on completion.
...
Ensures the index only returns active (unspent) UTXOs when queried. Without this, spent UTXOs accumulate in the cache and are incorrectly returned by query methods, causing transaction building failures. Applied when processing transactions or implementing query methods.
 ...
 - **REQT-1.3.5.1**/hhbcnvd9aj: IMPLEMENTED/NEEDS VERIFICATION: **Input Detection** - When processing a transaction via `processTransactionForNewUtxos()`, MUST examine `tx.body.inputs` to identify spent UTXOs. For each input whose utxoId exists in the cache: if the UTXO is unspent, MUST mark it as spent by the confirmed tx. If the UTXO is already marked `spentInTx` by a *different* transaction (a pending tx that claimed it speculatively), MUST overwrite `spentInTx` with the confirmed tx hash (on-chain reality wins) and MUST record the contention on the losing pending tx's `PendingTxEntry` via its `contestedByTxs` array, including the confirmed tx hash and block height.
...
Maintains a reliable record of which blocks have been fully processed for transactions, independent of the chain tip. Ensures the indexer can resume processing from the correct point after restart, and that no block's transactions are skipped. Applied when modifying sync resumption, block processing order, or transaction discovery.
 ...
 - **REQT-1.3.7.1**/5d4f73c9bf: IMPLEMENTED/NEEDS VERIFICATION: **Last Processed Block** — MUST track which blocks have been fully processed for transactions. MUST be able to determine the last processed block reliably after restart. The processing cursor MUST be independent of the chain tip (`lastBlockHeight`). In incremental mode, blocks MUST be processed in order — no block may be skipped. In catchup mode, the cursor advances to the chain tip after address-level reconciliation completes — no per-block processing records are stored.
...
Documents planned features and performance improvements not yet implemented. Applied when planning future development cycles, evaluating architectural extensions, or prioritizing feature roadmap.
 ...
 - **REQT-1.5.2**/zzsg63b2fb: IMPLEMENTED/NEEDS VERIFICATION: **Automated Periodic Refresh** - Must implement block-tip polling at a short interval (e.g. 5 seconds) to detect new blocks. When a new block is detected, MUST trigger `runSyncCycle()` (was `checkForNewTxns()`). MUST guard against concurrent sync — if `runSyncCycle` is already running, the poll skips. When the sync mutex per REQT/3c2s5nryvn (Sync Mutex) is implemented, MUST only trigger sync if this tab owns the mutex; non-owning tabs MUST poll the mutex for staleness instead.
...
Ensures the CachedUtxoIndex can be used as a drop-in replacement for Helios network clients, enabling cached UTXO lookups during transaction building. Applied when implementing or modifying the public API to match Helios interface contracts.
 ...
 - **REQT-1.6.7**/gz9a5b8qv: IMPLEMENTED/NEEDS VERIFICATION: **now Property** - MUST implement `now: number` property returning wall-clock milliseconds since epoch (i.e. `Date.now()`), matching the `CardanoClient` interface contract as implemented by `BlockfrostV0Client`.
...
Enables the indexer to cache UTXOs from connected wallet addresses, not just the Capo address. This allows the CapoDappProvider to serve actor UTXO queries from cache, reducing network calls and improving responsiveness when finding user-specific UTXOs for transaction building. Applied when implementing wallet integration or modifying address registration logic.
 ...
 - **REQT-1.9.5**/ygvc43wg0x: NEXT: **Wallet Transaction Monitoring** — During each periodic sync cycle (`runSyncCycle`), MUST also poll Blockfrost `addresses/{walletAddr}/transactions` for each registered wallet address (from `getAllWalletAddresses()`). MUST process discovered transactions through the existing `processTransactionForNewUtxos()` path, so wallet UTxOs receive proper `spentInTx` with real txHashes — the same correctness guarantees as Capo address UTxOs. MUST track per-address `lastBlockHeight` for incremental polling.
 - **REQT-1.9.6**/06b01nyf51: NEXT: **Wallet Snapshot Sync as Secondary Defense** — `syncWalletAddressIfStale()` remains as the on-demand freshness path when `getUtxos(walletAddress)` is called. After fetching fresh UTxOs from the network, MUST remove cached entries for that address whose `utxoId` is absent from the fresh set, then store the fresh entries. This serves as a secondary defense complementing the transaction-based monitoring in REQT/ygvc43wg0x. When removing cached entries absent from the fresh network set, MUST skip UTXOs whose txHash (extracted from `utxoId`) is present in `pendingTxHashes` — these are speculative outputs from pending transactions not yet confirmed on-chain. When storing fresh entries, MUST preserve existing `spentInTx` markers set by pending transaction registration per REQT/p2ts24jbkg (Register Pending Transaction). DEPENDS on REQT/q2aqr5rh7g (Pending State Before Sync) for `pendingTxHashes` availability.
 - **REQT-1.9.7**/2tgp4rg4sg: NEXT: **Wallet Sync State Advancement** — After processing wallet address transactions during `runSyncCycle`, MUST update each wallet address's `lastBlockHeight` in the store to reflect the highest block processed. This ensures the next sync cycle only polls for new transactions.
...
Enables the indexer to reflect locally-submitted transactions in query results immediately, before they are confirmed on-chain. Prevents stale reads after writes, avoids double-spend attempts from the same session, and provides pending-state visibility to downstream consumers. Applied when implementing or modifying transaction registration, confirmation, rollback, or pending-state query logic.
 ...
 - **REQT-1.11.1**/p2ts24jbkg: IMPLEMENTED/NEEDS VERIFICATION: **Register Pending Transaction** — MUST implement `registerPendingTx(signedCborHex: string, opts: { description, id, parentId?, depth, moreInfo?, txName?, txCborHex, txd?, buildTranscript?, txStructure?, signedTxStructure? })`. MUST decode signed CBOR via existing `decodeTx()` pipeline. MUST persist a `PendingTxEntry` to the store with `status: "pending"` and deadline computed as `txValidityEnd + graceBuffer` anchored to chain time. MUST persist diagnostic fields (`buildTranscript`, `txStructure`, `signedTxStructure`) when provided per REQT/vdkanffv9e (Diagnostic Fields). MUST mark each `tx.body.input` as spent by setting `spentInTx` to the pending txHash. MUST index each `tx.body.output` via existing `indexUtxoFromOutput()`. MUST parse inline datums into records if a Capo is attached per REQT/yx0yze9swf (Optional Capo Attachment). MUST store the live `txd` (TxDescription) in the in-memory `pendingTxMap` for same-session consumers. MUST be called once per transaction, in submission order, after successful network submission. MUST wrap its full write sequence (pending entry, input marking, output indexing, record parsing, tx CBOR) in `withWriteLock` per REQT/phj1ne600k (Callers).

 - **REQT-1.11.2**/58b9nzgcbj: IMPLEMENTED/NEEDS VERIFICATION: **Confirm Pending Transaction** — When a txHash matching a PendingTxEntry with `status: "pending"` is discovered on-chain — either via block processing (fast-path during `processTransactionForNewUtxos`) or via on-chain verification per REQT/ba6eq9x10s (On-Chain Pending Verification) — MUST confirm the entry and ensure its outputs are indexed. Sub-requirements below govern each concern.
    - **REQT-1.11.2.1**/05lkfji14c: IMPLEMENTED/NEEDS VERIFICATION: **Output Verification on Confirm** — MUST verify that all transaction outputs are indexed in the store, indexing any that are missing via `indexUtxoFromOutput()`. `registerPendingTx` writes outputs non-atomically; page reload mid-write can leave outputs partially indexed. The confirmation path MUST NOT assume outputs are present — it MUST check each and index any that are missing. When an output already exists in the store with a `spentInTx` marker, MUST NOT overwrite that marker (a later transaction in the same batch may have already consumed it).
    - **REQT-1.11.2.2**/klknoqvg4k: IMPLEMENTED/NEEDS VERIFICATION: **Depth-Accurate confirmState** — MUST calculate `confirmState` from actual block depth at the time of confirmation — not hardcoded to `"provisional"`, since the tx may already be many blocks deep (e.g. after page reload or catchup). When `confirmState` is past `"provisional"`, MUST remove from `pendingTxHashes`; otherwise MUST retain until `confirmState` reaches `"likely"` per REQT/pgyqdvwn17 (pendingTxHashes Lifecycle). Subsequent depth progression is governed by REQT/ddzcp753jr (Confirmation Depth Tracking).
    - **REQT-1.11.2.3**/1doel3r4ol: IMPLEMENTED/NEEDS VERIFICATION: **Confirmation Metadata Resolution** — MUST record the confirming block height, slot, and block hash (`confirmedInBlockHash`) for precise rollback matching per REQT/xsvqyh5gwb (confirmedInBlockHash Field). MUST resolve `confirmedAtSlot` and `confirmedInBlockHash` via store lookup with Blockfrost fallback when the confirming block is not in the block store.
    - **REQT-1.11.2.4**/y53cwz1dk4: IMPLEMENTED/NEEDS VERIFICATION: **Confirmation Event** — MUST emit `txConfirmed` event with `{ txHash, description, confirmState, txd? }` — `txd` available from in-memory map if same session.

 - **REQT-1.11.3**/a9y19g0pmr: IMPLEMENTED/NEEDS VERIFICATION: **Pending Transaction Rollback Lifecycle** — Governs the full lifecycle of rolling back a pending transaction that failed to confirm on-chain. The rollback proceeds through two gates: expiry detection (transition to `rollback-pending`) and contention settlement (wait for competing confirmed txs to reach sufficient depth), followed by safe rollback execution that honors on-chain reality.
    - **REQT-1.11.3.1**/vhn7zvn8nc: IMPLEMENTED/NEEDS VERIFICATION: **Expire to Rollback-Pending** — `checkPendingDeadlines()` MUST run on a 10-second timer, separate from the sync cycle timer. MUST query pending entries whose deadline slot is at least `confirmationThresholds.provisionalDepth` blocks behind `lastBlockHeight` (ensuring the chain has moved past the deadline with enough depth that it's not coming back). For each expired entry, MUST set status to `"rollback-pending"`. MUST NOT perform the actual rollback in this step.
    - **REQT-1.11.3.2**/bqy3xpp8rs: IMPLEMENTED/NEEDS VERIFICATION: **Contention Settlement Check** — MUST implement a rollback executor that runs each sync cycle for entries with `status === "rollback-pending"`. For each entry: if `contestedByTxs` is non-empty, MUST check that all competing txs have `lastBlockHeight - entry.blockHeight >= confirmationThresholds.provisionalDepth`. If any competing tx is too shallow, MUST skip and re-check next cycle. When all contention is settled (or no contention exists), MUST proceed to rollback execution (REQT/348pmgpdzr through REQT/szmt1wm6ef).
    - **REQT-1.11.3.3**/348pmgpdzr: IMPLEMENTED/NEEDS VERIFICATION: **Safe spentInTx Clearing** — MUST clear `spentInTx` on UTXOs where `spentInTx === txHash` (restore only uncontested inputs). Contested inputs already have `spentInTx` overwritten to the confirmed tx hash at detection time per REQT/hhbcnvd9aj and MUST NOT be touched.
    - **REQT-1.11.3.4**/c919hzrr2y: IMPLEMENTED/NEEDS VERIFICATION: **Pending Output and Record Cleanup** — MUST delete UTXO entries where `utxoId` starts with `txHash#` (remove speculative outputs). MUST delete record entries where `utxoId` starts with `txHash#` (remove pending-origin records). Confirmed tx outputs have different tx hashes and are unaffected.
    - **REQT-1.11.3.5**/1afcyedaks: IMPLEMENTED/NEEDS VERIFICATION: **Selective Datum Re-parse** — MUST re-parse inline datums ONLY from input UTXOs whose `spentInTx` was cleared in REQT/348pmgpdzr (the uncontested ones). MUST NOT re-parse inputs whose `spentInTx` points to a confirmed tx — those were legitimately spent and their successor records are already correctly indexed by `processTransactionForNewUtxos()`. Re-parsing requires Capo attached; MUST be a no-op without Capo (consistent with registration).
    - **REQT-1.11.3.6**/szmt1wm6ef: IMPLEMENTED/NEEDS VERIFICATION: **Rollback Finalization** — MUST set PendingTxEntry status to `"rolled-back"`. MUST emit `txRolledBack` event with `{ txHash, description, cbor, txd? }` — `cbor` included for recovery path. MUST clean up in-memory state (`pendingTxMap`, `pendingSpentUtxoIds`).

 - **REQT-1.11.4**/c3ytg4rttd: IMPLEMENTED/NEEDS VERIFICATION: **Deadline Calculation** — Deadline MUST be computed as `txValidityEnd + graceBuffer` where `txValidityEnd` is extracted from the decoded tx's validity interval and `graceBuffer` accounts for block propagation delay (e.g. 40-60 seconds). Deadline MUST be compared using block depth: the deadline slot must be at least `confirmationThresholds.provisionalDepth` blocks behind `lastBlockHeight`, ensuring the chain has definitively moved past the deadline. This prevents premature rollback from shallow blocks or clock drift.
 ...
 - **REQT-1.11.9**/fn70x96nxm: IMPLEMENTED/NEEDS VERIFICATION: **Startup Recovery** — On page reload, MUST load PendingTxEntry rows with `status === "pending"` from Dexie and expose them immediately with `pendingSyncState: "stale"`. After first sync cycle: `runSyncCycle()` MUST confirm any that landed in blocks — block processing provides fast-path confirmation for txs in newly-processed blocks, and `tryConfirmPendingTxs()` per REQT/ba6eq9x10s (On-Chain Pending Verification) catches any remaining unconfirmed entries (e.g. when the confirming block was already processed in a prior session). `checkPendingDeadlines()` MUST roll back any that expired; `resolvePendingState()` MUST trigger quiet resubmission for all surviving pending entries per REQT/qdrr0es7bm (Resubmission on Startup). MUST emit events for transitions. MUST flip `pendingSyncState` to `"fresh"` and emit `pendingSynced`. In-memory `pendingTxMap` is empty after reload — events fire without live `txd`; consumers get txHash, description, and cbor from Dexie fields.
    - **REQT-1.11.9.1**/q2aqr5rh7g: NEXT: **Pending State Before Sync** — `syncNow()` MUST populate `pendingTxHashes` and `pendingSpentUtxoIds` via `loadPendingFromStore()` BEFORE any sync activity (incremental, catchup, or wallet reconciliation) executes. This ordering ensures that reconciliation guards in REQT/06b01nyf51 (Wallet Snapshot Sync) and REQT/2he55bafxd (Catchup Mode) have pending-tx data available when they run.
    - **REQT-1.11.9.2**/ba6eq9x10s: IMPLEMENTED/NEEDS VERIFICATION: **On-Chain Pending Verification** — `tryConfirmPendingTxs()` MUST run within `runSyncCycle()` after chain state sync (step 3) and before resubmission (step 5). For each txHash in `pendingTxHashes` whose PendingTxEntry still has `status === "pending"`, MUST query Blockfrost `txs/{txHash}` to check on-chain status. If the tx is on-chain: MUST call `confirmPendingTx(txHash, block_height)` using the block height from the Blockfrost response. If the tx returns 404: MUST skip — resubmission and deadline paths handle unconfirmed txs. MUST handle errors gracefully (log and continue to next entry). This is the safety net that fulfills REQT/fn70x96nxm's confirmation contract when block processing does not encounter the tx (e.g. after page reload when the confirming block was already processed in a prior session). In normal operation (no reload gap), this step is a no-op — block processing's fast-path confirm in `processTransactionForNewUtxos` already handles them.
 ...
 - **REQT-1.11.12**/ddzcp753jr: IMPLEMENTED/NEEDS VERIFICATION: **Confirmation Depth Tracking** — MUST track confirmation depth for confirmed pending transactions as the chain advances. `confirmState` progresses through graduated confidence levels based on how many blocks have been built on top of the confirming block. When `confirmState` advances to `"likely"`, MUST remove the txHash from `pendingTxHashes` per REQT/pgyqdvwn17 (pendingTxHashes Lifecycle).
    ...
    - **REQT-1.11.12.3**/b6h7km6h44: IMPLEMENTED/NEEDS VERIFICATION: **Rollback Interaction** — If a chain rollback (REQT/yasww6cqa4) invalidates a confirmed tx's block, MUST revert `confirmState` to `undefined` and status back to `"pending"`. MUST increment `forkRecoveryCount` per REQT/k55zssabq2 (forkRecoveryCount Field). MUST re-add txHash to `pendingTxHashes`. MUST NOT touch UTxOs or records — speculative state from registration is still correct. Depth tracking and quiet resubmission resume automatically.

 - **REQT-1.11.13**/kfemj6eteg: IMPLEMENTED/NEEDS VERIFICATION: **Block-Discovered PendingTxEntry Creation** — `processTransactionForNewUtxos()` MUST create a `PendingTxEntry` for every transaction it processes from a confirmed block, unless one already exists (self-submitted txs already have entries from `registerPendingTx()`). Entry MUST be created with `status: "confirmed"`, `confirmState` based on current depth, `confirmedInBlockHash` from the block being processed, `confirmedAtBlockHeight`, `confirmedAtSlot`, and `signedTxCborHex` fetched from Blockfrost via `blocks/{hash}/txs/cbor` (batch) or `txs/{hash}/cbor` (individual). Fields not available for block-discovered txs MUST use sensible defaults (no type changes to `PendingTxEntry`): `description`: `"tx discovered from on-chain data"`, `id`: txHash, `batchDepth`: `86` (sentinel — identifies block-discovered origin), `txCborHex`: signed CBOR from Blockfrost (same as `signedTxCborHex`), `deadlineSlot`: parsed from CBOR validity interval + graceBuffer. Optional fields (`parentId`, `txName`, `buildTranscript`, `txStructure`) remain undefined. This ensures all txs in the index have uniform tracking for rollback resolution per REQT/2grpnzb2q0 (Resolve Pending Tx Confirmations).
...
Detects when previously-confirmed blocks are no longer on the canonical chain (orphaned/rolled back) and recovers the index to a consistent state. Cardano rollbacks are rare but can theoretically occur up to ~1 hour after apparent confirmation. Without detection, the index would contain UTxOs and records from blocks that no longer exist on-chain. Applied when modifying block verification, sync startup, or index consistency logic.
 ...
 - **REQT-1.12.1**/yasww6cqa4: IMPLEMENTED/NEEDS VERIFICATION: **Chain Intersection Discovery** — MUST verify chain continuity by fetching `blocks/{tipHash}/previous?count=100` from Blockfrost to obtain the canonical chain's recent block hashes. MUST sort the response by height ascending. MUST compare each canonical block hash against the stored block at that height via `store.findBlockByHeight(height)`. When a stored block exists and its hash differs from the canonical hash, MUST identify the stored block hash as rolled back. MUST collect all rolled-back block hashes for rollback execution.

 - **REQT-1.12.2**/4j3rs4pyjt: IMPLEMENTED/NEEDS VERIFICATION: **Rollback Recovery** — When rolled-back blocks are detected, MUST recover the index to a consistent state aligned with the canonical chain. Recovery MUST handle all index artifacts affected by transactions in the rolled-back blocks.
    - **REQT-1.12.2.1**/2grpnzb2q0: IMPLEMENTED/NEEDS VERIFICATION: **Resolve Pending Tx Confirmations** — For all txs with `confirmedInBlockHash ∈ rolledBackBlockHashes`, MUST scan all canonical replacement blocks' transaction lists for each tx hash. **Re-anchor path** (common): if the tx hash is found in a canonical block, MUST update `confirmedInBlockHash`, `confirmedAtBlockHeight`, `confirmedAtSlot` to the canonical block's values and recalculate `confirmationBlockDepth` and `confirmState` from the new height. **Revert path** (uncommon): if the tx hash is NOT found in any canonical replacement block, MUST revert via `revertConfirmedPendingTx()`: set `status` back to `"pending"`, clear `confirmedAtBlockHeight`, `confirmedAtSlot`, `confirmedInBlockHash`, `confirmState`, reset `confirmationBlockDepth` to 0, increment `forkRecoveryCount`, re-add to `pendingTxHashes`. MUST NOT touch UTxOs or records in either path — speculative state remains correct. Applies uniformly to self-submitted and block-discovered txs.
    - **REQT-1.12.2.2**/epwp74mn8x: IMPLEMENTED/NEEDS VERIFICATION: **Roll Back Processing Cursor** — MUST mark each rolled-back block as `state: "rolled back"` via `store.saveBlock()`. MUST save the canonical replacement block as `state: "unprocessed"` so normal sync processing handles it on the next pass.
    - **REQT-1.12.2.3**/j4198pv9ea: IMPLEMENTED/NEEDS VERIFICATION: **Re-initiate Sync** — Normal sync processing handles canonical replacement blocks (saved as `"unprocessed"` by REQT/epwp74mn8x) on the next pass. No special re-initiation mechanism needed — the existing `syncIncremental`/`syncCatchup` paths pick up unprocessed blocks automatically.

 - **REQT-1.12.3**/dnr06r6ch5: IMPLEMENTED/NEEDS VERIFICATION: **Rollback Event** — MUST emit a `chainRollback` event with rollback depth when a rollback is detected, so downstream consumers (CapoDappProvider, UI) can respond.
...
### REQT-1.14/cbpw1a6j8q: IMPLEMENTED/NEEDS VERIFICATION: **Quiet Transaction Resubmission**

#### Purpose
Proactively resubmits unconfirmed and shallowly-confirmed transactions to the Cardano network on each sync cycle, ensuring transactions survive micro-fork orphaning without waiting for rollback detection. This is a standing resilience behavior that runs independently of rollback detection. Applied when modifying the sync loop, pending tx lifecycle, or submission path logic.

 - **REQT-1.14.1**/qdrr0es7bm: IMPLEMENTED/NEEDS VERIFICATION: **Resubmission on Startup** — `resolvePendingState()` MUST trigger quiet resubmission for all surviving pending entries after page reload, before flipping `pendingSyncState` to `"fresh"`.

 - **REQT-1.14.2**/z9d167q2mw: IMPLEMENTED/NEEDS VERIFICATION: **Resubmission Loop** — MUST implement `resubmitStalePendingTxs()` called from `runSyncCycle()` each sync cycle, after chain state sync and pending tx verification, and before `updateConfirmationDepths()`. MUST iterate all txHashes in `pendingTxHashes`, load each `PendingTxEntry` from the store, and call `resubmitTx()` for eligible entries per REQT/bq1nmd4c8x (Resubmission Scope).

 - **REQT-1.14.3**/bq1nmd4c8x: IMPLEMENTED/NEEDS VERIFICATION: **Resubmission Scope** — A pending tx is eligible for resubmission when ALL of: (1) `status === "pending"` OR (`status === "confirmed"` AND `confirmState === "provisional"`), (2) validity window still open (`lastValidSlot >= lastBlockSlot`), (3) deadline block has NOT reached `"likely"` depth, (4) `lastResubmitAt` was at least 10 seconds ago per REQT/sg0pqr0dx7 (Resubmission Throttle).

 - **REQT-1.14.4**/sg0pqr0dx7: IMPLEMENTED/NEEDS VERIFICATION: **Resubmission Throttle** — MUST NOT resubmit a transaction more frequently than once per 10 seconds. MUST track the last resubmission attempt time via `lastResubmitAt` on `PendingTxEntry` (persisted to Dexie for cross-reload throttling). MUST update `lastResubmitAt` after each resubmission attempt.

 - **REQT-1.14.5**/zyw6grz9t1: IMPLEMENTED/NEEDS VERIFICATION: **Submission Path Abstraction** — MUST implement `resubmitTx(signedCborHex: string)` as a private method that decodes the CBOR and calls `this.network.submitTx()`. Designed for future extensibility: when Ogmios or Dred submitters are added, this method grows to route through multiple submission paths without changing the resubmission loop.

 - **REQT-1.14.6**/zhgbnajdjg: IMPLEMENTED/NEEDS VERIFICATION: **Harmless Error Handling** — `resubmitTx()` MUST swallow `SubmissionUtxoError` (tx already consumed its inputs — success case) and `SubmissionExpiryError` (tx will be rolled back by deadline). MUST log unexpected errors as warnings without interrupting the resubmission loop.

### REQT-1.15/xsvqyh5gwb: IMPLEMENTED/NEEDS VERIFICATION: **confirmedInBlockHash Field**

#### Purpose
Records which block hash a pending transaction was confirmed in, enabling precise rollback matching. Without this field, rollback reversion must match on block height alone, which is ambiguous when multiple blocks exist at the same height. Applied when modifying confirmation, rollback detection, or PendingTxEntry schema.

 - MUST add `confirmedInBlockHash` (string, optional) to `PendingTxEntry`. MUST be set in `confirmPendingTx()` from the block being processed. MUST be used by REQT/2grpnzb2q0 (Revert Pending Tx Confirmations) to match `confirmedInBlockHash === rolledBackBlockHash`. MUST be cleared on revert.

### REQT-1.16/k55zssabq2: IMPLEMENTED/NEEDS VERIFICATION: **forkRecoveryCount Field**

#### Purpose
Diagnostic counter tracking how many times a pending transaction's confirming block was rolled back and the tx reverted to pending. Used by UI (PendingTxTracker) to display fork recovery status. Applied when modifying rollback reversion or pending tx UI display.

 - MUST add `forkRecoveryCount` (number, optional, default 0) to `PendingTxEntry`. MUST be incremented by `revertConfirmedPendingTx()` each time a confirmed entry is reverted due to a block rollback. UI MAY display a fork recovery indicator when `forkRecoveryCount > 0`.

### REQT-1.17/pgyqdvwn17: IMPLEMENTED/NEEDS VERIFICATION: **pendingTxHashes Lifecycle**

#### Purpose
Changes `pendingTxHashes` membership to retain entries until confirmation reaches "likely" depth instead of removing on first confirmation. This ensures provisionally-confirmed txs remain eligible for quiet resubmission during the micro-fork vulnerability window. Applied when modifying `confirmPendingTx()`, `updateConfirmationDepths()`, or `isPending()` logic.

 - `confirmPendingTx()` MUST NOT remove from `pendingTxHashes`. `updateConfirmationDepths()` MUST remove from `pendingTxHashes` when `confirmState` advances to `"likely"`. `isPending()` MUST return truthy for provisionally-confirmed txs (acceptable behavioral change — they are not yet reliably confirmed). `loadPendingFromStore()` MUST continue to populate `pendingTxHashes` from Dexie on startup.

### REQT-1.18/3c2s5nryvn: IMPLEMENTED/NEEDS VERIFICATION: **Sync Mutex (Multi-Tab Coordination)**

#### Purpose
Prevents redundant and contentious sync work when multiple browser tabs share the same IndexedDB. Exactly one tab owns sync responsibility; other tabs read from cache. Applied when modifying `startPeriodicRefresh()`, `runSyncCycle()`, or any code that initiates sync activity.

 - **REQT-1.18.1**/37y4v8z8bq: IMPLEMENTED/NEEDS VERIFICATION: **Process ID Assignment** — Each browser tab MUST assign itself a random integer process ID (< ~2 billion) on construction. The pid MUST be used as the identity for mutex ownership.

 - **REQT-1.18.2**/f3w3hkjt4t: IMPLEMENTED/NEEDS VERIFICATION: **Mutex Acquisition** — On startup and when the mutex is stale or missing, a tab MUST attempt to acquire the sync mutex by writing its pid and current timestamp to the store's metadata table (key: `syncMutex`). Acquisition MUST use a compare-and-swap pattern: read the current mutex, verify it is absent or stale, then write the new value. If another tab acquired between read and write, the attempt fails and the tab resumes polling.

 - **REQT-1.18.3**/ekyatca2kq: IMPLEMENTED/NEEDS VERIFICATION: **Mutex Freshening** — The sync-owning tab MUST update the mutex timestamp during each sync cycle. The staleness threshold MUST be configurable with a default of 15 seconds.

 - **REQT-1.18.4**/hn1mv0cp7h: IMPLEMENTED/NEEDS VERIFICATION: **Non-Owner Behavior** — A tab that does not own the sync mutex MUST NOT execute `runSyncCycle()` or any sync-related network activity. It MUST continue serving reads from cached Dexie data. It MUST poll the mutex for staleness on the same interval as the block-tip poll (5s). When the mutex is stale, it MUST attempt acquisition per REQT/f3w3hkjt4t (Mutex Acquisition).

 - **REQT-1.18.5**/e0rzdrc7ts: IMPLEMENTED/NEEDS VERIFICATION: **Graceful Release** — On `beforeunload`, the owning tab SHOULD remove the `syncMutex` row from the metadata table. This is best-effort — the staleness timeout per REQT/ekyatca2kq (Mutex Freshening) is the primary recovery mechanism.

### REQT-1.19/jp1wtj9pfz: IMPLEMENTED/NEEDS VERIFICATION: **Write Lock (Multi-Step Write Atomicity)**

#### Purpose
Provides mutual exclusion for multi-step write operations across browser tabs, preventing interleaved writes that leave the store in inconsistent states (e.g., `registerPendingTx` saving a `PendingTxEntry` without all its outputs). All multi-step writes — from any tab, sync-owner or not — go through this uniform gate. Applied when adding or modifying any operation that performs multiple sequential store writes.

 - **REQT-1.19.1**/4tsvn6259v: IMPLEMENTED/NEEDS VERIFICATION: **withWriteLock API** — `UtxoStoreGeneric` MUST expose `withWriteLock<T>(pid: number, activityName: string, tables: string[], callback: () => Promise<T>): Promise<T>`. The method MUST acquire a write lock before executing the callback and release it after completion (or on error). The `tables` parameter declares which store tables the callback will write to.

 - **REQT-1.19.2**/jb5dhgsyfq: IMPLEMENTED/NEEDS VERIFICATION: **Lock Protocol** — Acquisition MUST write a lock row to the metadata table (key: `writeLock`) with pid, activityName, and timestamp. Release MUST remove the row. If the lock is held by another pid, the caller MUST retry with brief backoff (e.g., 10ms intervals). The lock MUST be considered stale after 1 second as a safety net for crashed tabs.

 - **REQT-1.19.3**/r7t394zt2x: IMPLEMENTED/NEEDS VERIFICATION: **Dexie Transaction Shim** — For the `DexieUtxoStore` implementation, `withWriteLock` MUST wrap the callback in a Dexie `db.transaction('rw', tables, callback)` to provide both mutual exclusion AND atomicity. The metadata-row protocol provides cross-tab mutual exclusion; the Dexie transaction provides intra-tab atomicity and rollback on error.

 - **REQT-1.19.4**/phj1ne600k: IMPLEMENTED/NEEDS VERIFICATION: **Callers** — `registerPendingTx()` MUST use `withWriteLock` to wrap its full write sequence (save pending entry, mark inputs spent, index outputs, parse records, save tx CBOR). `confirmPendingTx()` MUST use `withWriteLock` to wrap output verification and status update. Other multi-step write operations (rollback execution, reconciliation) SHOULD migrate to `withWriteLock` incrementally.

 - **REQT-1.19.5**/n11m1k9wvg: IMPLEMENTED/NEEDS VERIFICATION: **Non-Transactional Store Compatibility** — For store implementations without native transactions (e.g., future Dred or memory stores), the metadata-row lock protocol alone MUST provide sufficient mutual exclusion. The `withWriteLock` contract MUST NOT require the underlying store to support transactions.
...
Extends the storage interface with operations for persisting and managing in-flight transaction state. Applied when implementing pending-tx support in new storage backends or modifying the pending-tx persistence contract.
 ...
 - **REQT-2.2.4**/j5pwm8btay: IMPLEMENTED/NEEDS VERIFICATION: **Append Submission Log** — MUST add to `UtxoStoreGeneric`: `appendSubmissionLog(txHash: string, entry: SubmissionLogEntry): Promise<void>` to append a single `SubmissionLogEntry` to the `submissionLog` array of the `PendingTxEntry` identified by `txHash`. MUST read the current entry, append the new log entry, and write back atomically. Used by `TxSubmitMgr` (via callback) to persist submission events incrementally without coupling to the storage layer.

### REQT-2.3/4tsvn6259v: IMPLEMENTED/NEEDS VERIFICATION: **Write Lock Interface**

#### Purpose
Extends the storage interface with a cross-tab write lock for multi-step write atomicity. Applied when implementing `withWriteLock` in new storage backends or when adding new multi-step write operations to the indexer.

 - MUST add `withWriteLock<T>(pid: number, activityName: string, tables: string[], callback: () => Promise<T>): Promise<T>` to `UtxoStoreGeneric`. See REQT/jp1wtj9pfz (Write Lock) for behavioral requirements. Store implementations MUST provide at minimum the metadata-row mutual exclusion protocol; transactional stores (e.g., Dexie) SHOULD additionally wrap the callback in a native transaction per REQT/r7t394zt2x (Dexie Transaction Shim).
...
Defines the storage type for in-flight transaction records. Applied when modifying the pending-tx persistence schema or adding fields to the serializable projection of TxDescription.
 ...
 - **REQT-5.3.1**/jqz2m497vx: IMPLEMENTED/NEEDS VERIFICATION: **PendingTxEntry Type** — MUST define `PendingTxEntry` in `types/PendingTxEntry.ts` with fields: `txHash` (string, PK), `description` (string), `id` (string, TxDescription.id), `parentId` (string, optional), `depth` (number), `moreInfo` (string, optional), `txName` (string, optional), `txCborHex` (string, unsigned CBOR), `signedTxCborHex` (string, signed CBOR), `deadline` (number, epoch ms — txValidityEnd + graceBuffer based on chain time), `status` (`"pending" | "confirmed" | "rollback-pending" | "rolled-back"`), `contestedByTxs` (`Array<{ txHash: string, blockHeight: number }>`, optional, default empty — populated by contention detection per REQT/hhbcnvd9aj), `submittedAt` (number, epoch ms), `confirmedInBlockHash` (string, optional — per REQT/xsvqyh5gwb), `forkRecoveryCount` (number, optional — per REQT/k55zssabq2), `lastResubmitAt` (number, optional — per REQT/zhgbnajdjg). MUST be Helios-free and IndexedDB-serializable. MUST NOT include `BuiltTcxStats` (contains non-serializable `Wallet`, `WalletHelper`, `PubKeyHash` objects). The live `TxDescription` object is held in-memory only per REQT/2w2yyc2m1k (In-Memory Pending Map).

 - **REQT-5.3.2**/vdkanffv9e: IMPLEMENTED/NEEDS VERIFICATION: **Diagnostic Fields** — MUST extend `PendingTxEntry` with optional diagnostic fields for post-reload inspection: `buildTranscript` (`string[]`, snapshot of `tcx.logger.formattedHistory` at registration), `txStructure` (`string`, `dumpAny(tx, networkParams)` output captured at registration). All fields MUST be IndexedDB-serializable strings. These fields provide the data for the detail panel's transcript and structure tabs when live `TxSubmissionTracker` objects are unavailable (after page reload or tracker destruction).

 - **REQT-5.3.3**/h5jhpxf9c8: IMPLEMENTED/NEEDS VERIFICATION: **Submission Log** — MUST extend `PendingTxEntry` with `submissionLog` (`Array<SubmissionLogEntry>`, optional, default empty). `SubmissionLogEntry` MUST contain: `at` (number, epoch ms), `event` (string — e.g. `"submit-attempt"`, `"submit-success"`, `"submit-failed"`, `"confirm-attempt"`, `"confirmed"`, `"not-confirmed"`, `"backoff"`, `"tx-expired"`, `"rollback-pending"`, `"rolled-back"`), `submitter` (string, optional — submitter name when event is per-submitter), `detail` (string, optional — error message or state info). MUST be written incrementally as `TxSubmitMgr` state transitions occur, persisted to Dexie via REQT/j5pwm8btay (Append Submission Log). The log provides a full submission transcript that survives page reload.
...
* DONE: `now` property returning `Date.now()` (wall-clock milliseconds since epoch), matching CardanoClient interface contract (REQT/gz9a5b8qv)
