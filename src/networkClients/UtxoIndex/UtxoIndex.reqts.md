...
Enables the indexer to reflect locally-submitted transactions in query results immediately, before they are confirmed on-chain. Prevents stale reads after writes, avoids double-spend attempts from the same session, and provides pending-state visibility to downstream consumers. Applied when implementing or modifying transaction registration, confirmation, rollback, or pending-state query logic.
 ...
 - **REQT-1.11.2**/58b9nzgcbj: IMPLEMENTED/NEEDS VERIFICATION: **Confirm Pending Transaction** — When a txHash matching a PendingTxEntry with `status: "pending"` is discovered on-chain — either via block processing (fast-path during `processTransactionForNewUtxos`) or via on-chain verification per REQT/ba6eq9x10s (On-Chain Pending Verification) — MUST skip normal indexing (outputs already indexed, inputs already marked spent). MUST set the entry's `confirmState` to `"provisional"` and record the confirming block height. MUST record the confirming block hash in `confirmedInBlockHash` for precise rollback matching per REQT/xsvqyh5gwb (confirmedInBlockHash Field). MUST emit `txConfirmed` event with `{ txHash, description, confirmState, txd? }` — `txd` available from in-memory map if same session. MUST NOT remove the txHash from `pendingTxHashes` — entries remain pending until `confirmState` reaches `"likely"` per REQT/pgyqdvwn17 (pendingTxHashes Lifecycle). Subsequent depth progression is governed by REQT/ddzcp753jr (Confirmation Depth Tracking).
