...
### REQT-1.18/3c2s5nryvn: NEXT: **Sync Mutex (Multi-Tab Coordination)**
...
Prevents redundant and contentious sync work when multiple browser tabs share the same IndexedDB. Exactly one tab owns sync responsibility; other tabs read from cache. Applied when modifying `startPeriodicRefresh()`, `runSyncCycle()`, or any code that initiates sync activity.
 ...
 - **REQT-1.18.1**/37y4v8z8bq: NEXT: **Process ID Assignment** — Each browser tab MUST assign itself a random integer process ID (< ~2 billion) on construction. The pid MUST be used as the identity for mutex ownership.

 - **REQT-1.18.2**/f3w3hkjt4t: NEXT: **Mutex Acquisition** — On startup and when the mutex is stale or missing, a tab MUST attempt to acquire the sync mutex by writing its pid and current timestamp to the store's metadata table (key: `syncMutex`). Acquisition MUST use a compare-and-swap pattern: read the current mutex, verify it is absent or stale, then write the new value. If another tab acquired between read and write, the attempt fails and the tab resumes polling.

 - **REQT-1.18.3**/ekyatca2kq: NEXT: **Mutex Freshening** — The sync-owning tab MUST update the mutex timestamp during each sync cycle. The staleness threshold MUST be configurable with a default of 15 seconds.

 - **REQT-1.18.4**/hn1mv0cp7h: NEXT: **Non-Owner Behavior** — A tab that does not own the sync mutex MUST NOT execute `runSyncCycle()` or any sync-related network activity. It MUST continue serving reads from cached Dexie data. It MUST poll the mutex for staleness on the same interval as the block-tip poll (5s). When the mutex is stale, it MUST attempt acquisition per REQT/f3w3hkjt4t (Mutex Acquisition).

 - **REQT-1.18.5**/e0rzdrc7ts: NEXT: **Graceful Release** — On `beforeunload`, the owning tab SHOULD remove the `syncMutex` row from the metadata table. This is best-effort — the staleness timeout per REQT/ekyatca2kq (Mutex Freshening) is the primary recovery mechanism.

### REQT-1.19/jp1wtj9pfz: NEXT: **Write Lock (Multi-Step Write Atomicity)**
...
Provides mutual exclusion for multi-step write operations across browser tabs, preventing interleaved writes that leave the store in inconsistent states (e.g., `registerPendingTx` saving a `PendingTxEntry` without all its outputs). All multi-step writes — from any tab, sync-owner or not — go through this uniform gate. Applied when adding or modifying any operation that performs multiple sequential store writes.
 ...
 - **REQT-1.19.1**/4tsvn6259v: NEXT: **withWriteLock API** — `UtxoStoreGeneric` MUST expose `withWriteLock<T>(pid: number, activityName: string, tables: string[], callback: () => Promise<T>): Promise<T>`. The method MUST acquire a write lock before executing the callback and release it after completion (or on error). The `tables` parameter declares which store tables the callback will write to.

 - **REQT-1.19.2**/jb5dhgsyfq: NEXT: **Lock Protocol** — Acquisition MUST write a lock row to the metadata table (key: `writeLock`) with pid, activityName, and timestamp. Release MUST remove the row. If the lock is held by another pid, the caller MUST retry with brief backoff (e.g., 10ms intervals). The lock MUST be considered stale after 1 second as a safety net for crashed tabs.

 - **REQT-1.19.3**/r7t394zt2x: NEXT: **Dexie Transaction Shim** — For the `DexieUtxoStore` implementation, `withWriteLock` MUST wrap the callback in a Dexie `db.transaction('rw', tables, callback)` to provide both mutual exclusion AND atomicity. The metadata-row protocol provides cross-tab mutual exclusion; the Dexie transaction provides intra-tab atomicity and rollback on error.

 - **REQT-1.19.4**/phj1ne600k: NEXT: **Callers** — `registerPendingTx()` MUST use `withWriteLock` to wrap its full write sequence (save pending entry, mark inputs spent, index outputs, parse records, save tx CBOR). `confirmPendingTx()` MUST use `withWriteLock` to wrap output verification and status update. Other multi-step write operations (rollback execution, reconciliation) SHOULD migrate to `withWriteLock` incrementally.

 - **REQT-1.19.5**/n11m1k9wvg: NEXT: **Non-Transactional Store Compatibility** — For store implementations without native transactions (e.g., future Dred or memory stores), the metadata-row lock protocol alone MUST provide sufficient mutual exclusion. The `withWriteLock` contract MUST NOT require the underlying store to support transactions.
...
### REQT-2.3/4tsvn6259v: NEXT: **Write Lock Interface**
