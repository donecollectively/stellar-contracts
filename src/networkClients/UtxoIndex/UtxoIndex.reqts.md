...
Enables the indexer to cache UTXOs from connected wallet addresses, not just the Capo address. This allows the CapoDappProvider to serve actor UTXO queries from cache, reducing network calls and improving responsiveness when finding user-specific UTXOs for transaction building. Applied when implementing wallet integration or modifying address registration logic.
 ...
 - **REQT-1.9.5**/ygvc43wg0x: NEXT: **Wallet Sync Reconciliation** — When syncing a wallet address (both initial registration via `addWalletAddress()` and periodic refresh via `syncWalletAddressIfStale()`), MUST reconcile cached UTxOs against the fresh network snapshot. After fetching current UTxOs from the network, MUST identify cached unspent UTxOs for that address whose `utxoId` is absent from the fresh set. MUST mark those stale entries as spent via `spentInTx` (using a non-txHash sentinel such as `"reconciled"` to avoid collision with real transaction hashes or the in-flight transaction rollback path). MUST NOT hard-delete stale entries — soft delete via `spentInTx` is required per REQT/11msfc4wv8 (Soft Delete Strategy).
 - **REQT-1.9.6**/06b01nyf51: NEXT: **Stale UTXO Eviction on Wallet Sync** — After reconciliation per REQT/ygvc43wg0x, `getUtxos(walletAddress)` MUST return only UTxOs that exist in the most recent network snapshot. Stale UTxOs marked with `spentInTx` during reconciliation MUST be filtered out by existing query filtering (REQT/g3jen1rcvd). The edge case where a wallet has zero on-chain UTxOs MUST result in all cached entries for that address being marked spent.
...
Enables the indexer to reflect locally-submitted transactions in query results immediately, before they are confirmed on-chain. Prevents stale reads after writes, avoids double-spend attempts from the same session, and provides pending-state visibility to downstream consumers. Applied when implementing or modifying transaction registration, confirmation, rollback, or pending-state query logic.
 ...
 - **REQT-1.11.6**/r0y7s2vggr: IMPLEMENTED/NEEDS VERIFICATION: **getPendingTxs Query** — MUST implement `getPendingTxs(): PendingTxEntry[]` returning all entries with `status === "pending"`. Used by CapoDappProvider on startup to show pending count/list before sync completes.
