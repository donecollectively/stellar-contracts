...
Enables the indexer to reflect locally-submitted transactions in query results immediately, before they are confirmed on-chain. Prevents stale reads after writes, avoids double-spend attempts from the same session, and provides pending-state visibility to downstream consumers. Applied when implementing or modifying transaction registration, confirmation, rollback, or pending-state query logic.
 ...
 - **REQT-1.11.13**/kfemj6eteg: NEXT: **Block-Discovered PendingTxEntry Creation** — `processTransactionForNewUtxos()` MUST create a `PendingTxEntry` for every transaction it processes from a confirmed block, unless one already exists (self-submitted txs already have entries from `registerPendingTx()`). Entry MUST be created with `status: "confirmed"`, `confirmState` based on current depth, `confirmedInBlockHash` from the block being processed, `confirmedAtBlockHeight`, `confirmedAtSlot`, and `signedTxCborHex` fetched from Blockfrost via `blocks/{hash}/txs/cbor` (batch) or `txs/{hash}/cbor` (individual). Fields not available for block-discovered txs (`description`, `id`, `parentId`, `depth`, `txName`, `txCborHex`, `buildTranscript`, `txStructure`) MUST be optional on `PendingTxEntry` or filled with sensible defaults. This ensures all txs in the index have uniform tracking for rollback resolution per REQT/2grpnzb2q0 (Resolve Pending Tx Confirmations).
