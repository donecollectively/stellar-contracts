...
Detects when previously-confirmed blocks are no longer on the canonical chain (orphaned/rolled back) and recovers the index to a consistent state. Cardano rollbacks are rare but can theoretically occur up to ~1 hour after apparent confirmation. Without detection, the index would contain UTxOs and records from blocks that no longer exist on-chain. Applied when modifying block verification, sync startup, or index consistency logic.
 ...
 - **REQT-1.12.2**/4j3rs4pyjt: NEXT: **Rollback Recovery** — When rolled-back blocks are detected, MUST recover the index to a consistent state aligned with the canonical chain. Recovery MUST handle all index artifacts affected by transactions in the rolled-back blocks.
    - **REQT-1.12.2.1**/2grpnzb2q0: NEXT: **Resolve Pending Tx Confirmations** — For all txs with `confirmedInBlockHash ∈ rolledBackBlockHashes`, MUST scan all canonical replacement blocks' transaction lists for each tx hash. **Re-anchor path** (common): if the tx hash is found in a canonical block, MUST update `confirmedInBlockHash`, `confirmedAtBlockHeight`, `confirmedAtSlot` to the canonical block's values and recalculate `confirmationBlockDepth` and `confirmState` from the new height. **Revert path** (uncommon): if the tx hash is NOT found in any canonical replacement block, MUST revert via `revertConfirmedPendingTx()`: set `status` back to `"pending"`, clear `confirmedAtBlockHeight`, `confirmedAtSlot`, `confirmedInBlockHash`, `confirmState`, reset `confirmationBlockDepth` to 0, increment `forkRecoveryCount`, re-add to `pendingTxHashes`. MUST NOT touch UTxOs or records in either path — speculative state remains correct. Applies uniformly to self-submitted and block-discovered txs.
    - **REQT-1.12.2.2**/epwp74mn8x: NEXT: **Roll Back Processing Cursor** — MUST mark each rolled-back block as `state: "rolled back"` via `store.saveBlock()`. MUST save the canonical replacement block as `state: "unprocessed"` so normal sync processing handles it on the next pass.
    - **REQT-1.12.2.3**/j4198pv9ea: NEXT: **Re-initiate Sync** — Normal sync processing handles canonical replacement blocks (saved as `"unprocessed"` by REQT/epwp74mn8x) on the next pass. No special re-initiation mechanism needed — the existing `syncIncremental`/`syncCatchup` paths pick up unprocessed blocks automatically.
...
### REQT-1.14/cbpw1a6j8q: NEXT: **Quiet Transaction Resubmission**
...
Proactively resubmits unconfirmed and shallowly-confirmed transactions to the Cardano network on each sync cycle, ensuring transactions survive micro-fork orphaning without waiting for rollback detection. This is a standing resilience behavior that runs independently of rollback detection. Applied when modifying the sync loop, pending tx lifecycle, or submission path logic.
 ...
 - **REQT-1.14.1**/qdrr0es7bm: NEXT: **Resubmission on Startup** — `resolvePendingState()` MUST trigger quiet resubmission for all surviving pending entries after page reload, before flipping `pendingSyncState` to `"fresh"`.

 - **REQT-1.14.2**/z9d167q2mw: NEXT: **Resubmission Loop** — MUST implement `resubmitStalePendingTxs()` called from `checkForNewTxns()` each sync cycle, after block processing and before `updateConfirmationDepths()`. MUST iterate all txHashes in `pendingTxHashes`, load each `PendingTxEntry` from the store, and call `resubmitTx()` for eligible entries per REQT/bq1nmd4c8x (Resubmission Scope).

 - **REQT-1.14.3**/bq1nmd4c8x: NEXT: **Resubmission Scope** — A pending tx is eligible for resubmission when ALL of: (1) `status === "pending"` OR (`status === "confirmed"` AND `confirmState === "provisional"`), (2) validity window still open (`lastValidSlot >= lastBlockSlot`), (3) deadline block has NOT reached `"likely"` depth, (4) `lastResubmitAt` was at least 10 seconds ago per REQT/sg0pqr0dx7 (Resubmission Throttle).

 - **REQT-1.14.4**/sg0pqr0dx7: NEXT: **Resubmission Throttle** — MUST NOT resubmit a transaction more frequently than once per 10 seconds. MUST track the last resubmission attempt time via `lastResubmitAt` on `PendingTxEntry` (persisted to Dexie for cross-reload throttling). MUST update `lastResubmitAt` after each resubmission attempt.

 - **REQT-1.14.5**/zyw6grz9t1: NEXT: **Submission Path Abstraction** — MUST implement `resubmitTx(signedCborHex: string)` as a private method that decodes the CBOR and calls `this.network.submitTx()`. Designed for future extensibility: when Ogmios or Dred submitters are added, this method grows to route through multiple submission paths without changing the resubmission loop.

 - **REQT-1.14.6**/zhgbnajdjg: NEXT: **Harmless Error Handling** — `resubmitTx()` MUST swallow `SubmissionUtxoError` (tx already consumed its inputs — success case) and `SubmissionExpiryError` (tx will be rolled back by deadline). MUST log unexpected errors as warnings without interrupting the resubmission loop.

### REQT-1.15/xsvqyh5gwb: NEXT: **confirmedInBlockHash Field**
...
### REQT-1.16/k55zssabq2: NEXT: **forkRecoveryCount Field**
...
### REQT-1.17/pgyqdvwn17: NEXT: **pendingTxHashes Lifecycle**
